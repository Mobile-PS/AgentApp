...
import '../../ticket_dropdown_page_controller.dart';
import '../../utils/color_utils.dart';
import '../../utils/date_utils.dart';
import '../../utils/default_button1.dart';
...
class _TicketDetailsState extends State<TicketDetails> {
  ...
  MyController controller = Get.isRegistered() ? Get.find() : Get.put(MyController());
  BookingBloc bloc = BookingBloc(BusRouteRepository());

  final con = Get.put(TicketDropdownPageController());


  @override
  void initState() {

    Future.delayed(const Duration(seconds: 0), () =>  con.GetPayMode(controller.selectedBus!.routeId!,context));
  ...
  Widget build(BuildContext context) {
    var state;
    return GetBuilder<TicketDropdownPageController>(builder: (_)
    {
      return Scaffold(
          appBar: AppBar(
              toolbarHeight: 70,
                            ...
                            children: [
                              ...
                              boldTextField(
                                  "${controller.selectedBus
                                      ?.sourceName} - ${controller.selectedBus
                                      ?.destinationName}"),
                              simpleTextField("Name"),
                              boldTextField(
                                  "${controller.passangerList[0]
                                      .firstName} ${controller.passangerList[0]
                                      .lastName}"),
                              simpleTextField("NET Fare"),
                              boldTextField("${controller.selectedBus?.fare}"),
                              simpleTextField("Total Fare"),
                              boldTextField(
                                  "${controller.selectedBus?.fare * controller
                      ...
                      Expanded(
                        child: Column(
                          ...
                          children: [
                            ...
                            boldTextField(
                                "${MyDateUtils.getFormatterDate(
                                    controller.selectedBus!.date)}"),
                            simpleTextField("Email"),
                            boldTextField("${controller.passangerList[0]
                                .email}"),
                  ...
                  DropdownButtonHideUnderline(
                      child: DropdownButton2(
                          ...
                          onChanged: (value) {
                            ...
                            for(int i=0 ; i <con.selectedByDataList.length; i++ ){
                              if(con.selectedByDataList[i].channel == value){
                                setState(() {
                                  selectedPaymentType = value as String;
                                  payid = con.selectedByDataList[i].id!;
                                  position = i;
                                });
                              }
                            }
                      ...
                      child: BlocBuilder<BookingBloc, BookingState>(
                        ...
                        builder: (context, state) {
                          ...
                          if (state is BookingSuccessState) {
                           ...
                           // createDialog(context, res.refId.toString());
                          ...
                          return CustomLoadingButton(
                            ...
                            onPress: () {
                              ...
                              con.SavebookingRequest(context,payid!,selectedPaymentType,position!,BookingRequest(
                                  agentType: "bus_agent",
                                  ...
                                  primaryInfo: PrimaryInfo(
                                      ...
                                      passengerId: 0,
                                      paymentGateway: selectedPaymentType == 'Cash'?'cash':'Mobile Money',
                                      paymentSource: selectedPaymentType,
                                      pickPoint: controller
                                          .selectedBoardingPoint!.pointId,
                                      fkRouteid: controller.selectedBus
                                          ?.routeId,
                                      fkRouteViaCityId:
                                      controller.selectedBus?.routeViaCityId,
                                      seatCount: controller.selectedList
                                          .length,
                                      bseatsList: bseatList,
                                      startCityId: controller.source?.cityId,
                                      stopCityId: controller.destination
                                          ?.cityId,
                                      ...
                                      tripDate: controller.selectedBus
                                          ?.date)));
